...
class GPTConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        conclusion = await self.get_existing_conclusion(search_keyword, analysis_type)
        if conclusion:
            await self.send(text_data=json.dumps({"message": conclusion}))
        else:
    ...
    @database_sync_to_async
    def get_existing_conclusion(self, search_keyword, analysis_type):
        try:
            user_search = UserSearch.objects.get(
                user=self.scope["user"], search_word=search_keyword
            )
            if analysis_type == "distribution":
                return user_search.distribution_conclusion
            elif analysis_type == "innovation":
                return user_search.innovation_conclusion
            elif analysis_type == "network":
                return user_search.network_conclusion
